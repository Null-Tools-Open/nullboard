...
export function ImageOptionsPanel({ options, onOptionsChange, onClose }: ImageOptionsPanelProps) {
  const [localOptions, setLocalOptions] = useState<ImageOptions>(options)

  const updateOption = <K extends keyof ImageOptions>(key: K, value: ImageOptions[K]) => {
  ...
  return (
    <div className="fixed left-4 top-1/2 -translate-y-1/2 z-50 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-2xl shadow-xl p-4 w-64">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-zinc-100">Image</h3>
        {onClose && (
          <button
            onClick={onClose}
        ...
        <div className="flex gap-2">
          <button
            ...
            className={cn(
              ...
              localOptions.cornerStyle === 'sharp'
