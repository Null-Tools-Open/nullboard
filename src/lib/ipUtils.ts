...
const LEGACY_SALT = 'ip-encryption-salt'

function deriveKey(userId: string, salt: Buffer | string): Buffer {
  const keyMaterial = `${userId}:${IP_ENCRYPTION_SECRET}`
  const saltBuffer = typeof salt === 'string' ? Buffer.from(salt, 'utf8') : salt
  return crypto.pbkdf2Sync(keyMaterial, saltBuffer, 100000, 32, 'sha256')
}
...
export function decryptIp(encryptedIp: string, userId: string): string {
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      const iv = Buffer.from(ivHex, 'hex')
      const tag = Buffer.from(tagHex, 'hex')
      const key = deriveKey(userId, LEGACY_SALT)
