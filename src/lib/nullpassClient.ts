...
class NullpassClient {
  ...
  async getAvatar(token?: string, userId?: string, internalSecret?: string): Promise<Response> {
    const url = new URL(`${this.baseUrl}/api/avatar`)
  ...
  mapToNulldropUser(nullpassUser: NullpassUser): any {
    ...
    return {
      ...
      colabCursors: boardService?.metadata?.colabCursors ?? undefined,
    }
  }
}
